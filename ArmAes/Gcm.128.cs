...
public static partial class Gcm
{
        ...
        ref byte tag, nuint tagLen)
    {
        ...
        if (nonceLeft == 96 / 8)
        {
            ...
            tmp1 = x;
            tmp2 = y;
            AesUtil.Encrypt128(ref tmp1, ref tmp2, ref key.EncryptKeySchedule);
            h = tmp1;
            t = tmp2;
        ...
        {
            h = AesUtil.Encrypt128(ref key.EncryptKeySchedule, x);
            ...
            y = Ghash.Gfmul(y, h);
            y = Ghash.BswapVec(y);
            t = AesUtil.Encrypt128(ref key.EncryptKeySchedule, y);
        }
        ...
        while (ptLeft >= BytesPerBlock * 4)
        {
            ...
            AesUtil.Encrypt128(ref tmp1, ref tmp2, ref tmp3, ref tmp4, ref key.EncryptKeySchedule);
        ...
        while (ptLeft >= BytesPerBlock)
        {
            tmp1 = Ghash.Bswap64(ctr1);
            ctr1 = Vector128.Add(ctr1.AsUInt64(), One.AsUInt64()).AsByte();
            tmp1 = AesUtil.Encrypt128(ref key.EncryptKeySchedule, tmp1);
            tmp1 ^= Vector128.LoadUnsafe(ref pt);
            tmp1.StoreUnsafe(ref ct);
            tmp1 = Ghash.BswapVec(tmp1);
            x ^= tmp1;
    ...
    private static void AeDecrypt128(
        in AesKey key,
